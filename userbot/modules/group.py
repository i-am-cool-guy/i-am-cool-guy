...
async def demote(event):
  ...
  await Neo(EditAdminRequest(event.chat_id, user, rights, LANG['DEMOTED']))
...
async def pin(event):
  ...
  await Neo.pin_message(event.chat_id, event.reply_to_msg_id)
...
async def revoke(event):
  link = await Neo(ExportChatInviteRequest(event.chat_id))
...
async def open_chat(event):
  rights = ChatBannedRights(send_messages=False)
  await Neo(EditBannedRequest(event.chat_id, event.chat_id, rights))
